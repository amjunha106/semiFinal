...
%>
	<header> 
		...
		<c:when test="${not empty sessionScope.m_id}">
			...
			<button class="signup-button"  onclick="location.href='${path}/logout.do'">로그아웃</button>
		</c:when>
		<c:otherwise>
		<div class="user-buttons">
			<button class="login-button" onclick="location.href='http://localhost:7080/htproject/login.do'">로그인</button>
	...
	<main>
		<section class="search-form">
			<h2>열차 조회</h2>
			<form action="/htproject/t_info100.do" method="get">
				<input placeholder="출발지" name="train_depart" class=""/>
	   			<input placeholder="도착지" name="train_arrive" class=""/>
...
function goPage01(){
	  const username = "<%=m_id%>";
	  location.href = "http://localhost:7080/htproject/mem.do?m_id=<%=m_id%>";
}
